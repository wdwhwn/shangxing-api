...
public class OcCartController extends ApiBaseAction {
    @Autowired
...
/**
    ...
    @PostMapping("add")
    public Object add(String token,Integer goodsId,Integer attrId,Integer goodsNum) {
        TokenEntity tokenEntity = tokenService.queryByToken(token);
        Long mid = tokenEntity.getUserId();
        //判断商品是否可以购买
//        GoodsVo goodsInfo = goodsService.queryObject(goodsId);
        OcGoodsEntity ocGoodsEntity = ocGoodsService.queryObject(goodsId);
//        if (null == ocGoodsEntity || goodsInfo.getIs_delete() == 1 || goodsInfo.getIs_on_sale() != 1) {
        if (null == ocGoodsEntity || ocGoodsEntity.getStatus()!=1) {
            return this.toResponsObject(400, "商品已下架", "");
        }
        //取得规格的信息,判断规格库存
//        ProductVo productInfo = productService.queryObject(productId);
        OcGoodsAttributeEntity ocGoodsAttributeEntity = ocGoodsAttributeService.queryObject(attrId);
        if (null == ocGoodsAttributeEntity ||ocGoodsAttributeEntity.getSpecStock()<goodsNum ) {
            return this.toResponsObject(400, "库存不足", "");
        }
        //判断购物车中是否存在此规格商品
        Map cartParam = new HashMap();
        cartParam.put("goodsId", goodsId);
        cartParam.put("attrId", attrId);
        cartParam.put("mid",mid );
        ...
        if (null == cartInfo) {
            ...
            if (null != ocGoodsAttributeEntity.getSpecIds()&& ocGoodsAttributeEntity.getSpecIds().length() > 0) {
                Map specificationParam = new HashMap();
                String[] idsArray = getSpecificationIdsArray(ocGoodsAttributeEntity.getSpecIds());
            ...
            cartInfo.setProduct_id(attrId);
            cartInfo.setGoods_sn(goodsId+"");
            cartInfo.setGoods_name(ocGoodsEntity.getGoodsName());
            cartInfo.setList_pic_url(ocGoodsEntity.getImgIdArray());
            cartInfo.setNumber(goodsNum);
            cartInfo.setSession_id("1");
            cartInfo.setUser_id(mid);
            cartInfo.setRetail_price(ocGoodsEntity.getPrice());
            cartInfo.setMarket_price(ocGoodsEntity.getMarketPrice());
            if (null != goodsSepcifitionValue) {
                cartInfo.setGoods_specifition_name_value(StringUtils.join(goodsSepcifitionValue, ";"));
            }
            cartInfo.setGoods_specifition_ids(ocGoodsAttributeEntity.getSpecIds());
            cartInfo.setChecked(1);
            cartService.save(cartInfo);
        } else {
            //如果已经存在购物车中，则数量增加
            if (ocGoodsAttributeEntity.getSpecStock() < (goodsNum + cartInfo.getNumber())) {
                return this.toResponsObject(400, "库存不足", "");
            }
            cartInfo.setNumber(cartInfo.getNumber() + goodsNum);
            cartService.update(cartInfo);
        }
        UserVo userVo = new UserVo();
        return toResponsSuccess(getCart(userVo));
    }
    */
/**
     * 减少商品到购物车
